...
import {fxOptions,fxPalettes} from "@/lib/editor/visualFx";
...
it("replaces only the Visual FX select and applies the existing palette on selection",()=>{
 ...
 expect(screen.getAllByRole("option").filter(el=>el.closest('[role="listbox"]'))).toHaveLength(fxOptions.length+1);
...
it("supports arrows, Home/End, typeahead and Enter with selected indication",()=>{
 ...
 fireEvent.keyDown(trigger,{key:"End"});expect(document.getElementById(trigger.getAttribute("aria-activedescendant")!)?.textContent).toContain("Confetti");
...
it("keeps the menu open during internal scrolling and supports legacy values",()=>{
 ...
 expect(trigger.textContent).toContain("Impact / Explosion");fireEvent.click(trigger);fireEvent.scroll(screen.getByRole("listbox"));
 expect(screen.getByRole("option",{name:"Impact / Explosion"}).getAttribute("aria-selected")).toBe("true");
